...
def load_db_mapping():
    ...
    with open(DB_CONFIG_FILE, "w") as f:
        json.dump(DEFAULT_DB_MAPPING, f, indent=4)
    return DEFAULT_DB_MAPPING

def save_db_mapping(mapping):
    with open(DB_CONFIG_FILE, "w") as f:
...
class App:
    ...
    def database_selector_gui(self):
        ...
        self.db_win.title("DATABASE COURIER SELECTOR")
        self.db_win.geometry("450x420")
        self.popup_config(self.db_win)

        ctk.CTkLabel(self.db_win, text="PILIH DATABASE COURIER", font=("Segoe UI", 20, "bold")).pack(pady=20)

        self.db_options = list(DB_MAPPING.keys())
        self.db_choice = StringVar(value=self.db_options[0])
        
        self.dropdown = ctk.CTkOptionMenu(self.db_win, variable=self.db_choice, values=self.db_options, width=280, height=40)
        ...
        def load_selected_db():
            ...
            if self.connect_db():
                self.process_login(self.db_win)

        ctk.CTkButton(self.db_win, text="LOAD & MASUK DATABASE", font=("Segoe UI", 12, "bold"), width=280, height=45, fg_color="#388E3C", hover_color="#2E7D32", command=load_selected_db).pack(pady=15)
        
        # Fitur Baru: Tambah Database Baru
        ctk.CTkButton(self.db_win, text="➕ TAMBAH DATABASE BARU", font=("Segoe UI", 11, "bold"), width=280, height=35, fg_color="#1976D2", hover_color="#1565C0", command=self.add_new_database_ui).pack(pady=5)
        
        ctk.CTkButton(self.db_win, text="GANTI USER", font=("Segoe UI", 12, "bold"), width=280, height=40, fg_color="#E65100", hover_color="#F57C00", command=lambda: [self.db_win.destroy(), self.logout()]).pack(pady=15)

    def add_new_database_ui(self):
        win = ctk.CTkToplevel(self.db_win)
        win.title("TAMBAH DB BARU (ADMIN ONLY)")
        win.geometry("350x300")
        self.popup_config(win)

        ctk.CTkLabel(win, text="OTORISASI ADMIN", font=("Segoe UI", 12, "bold")).pack(pady=10)
        
        user_entry = ctk.CTkEntry(win, placeholder_text="Username Admin", width=250)
        user_entry.pack(pady=5)
        
        pass_entry = ctk.CTkEntry(win, placeholder_text="Password Admin", show="*", width=250)
        pass_entry.pack(pady=5)

        ctk.CTkLabel(win, text="NAMA DATABASE BARU", font=("Segoe UI", 12, "bold")).pack(pady=(15, 5))
        db_name_entry = ctk.CTkEntry(win, placeholder_text="Contoh: Ninja Express", width=250)
        db_name_entry.pack(pady=5)

        def verify_and_add():
            usr = user_entry.get().strip().lower()
            pw = pass_entry.get()
            new_db_name = db_name_entry.get().strip()

            if not new_db_name:
                messagebox.showerror("ERROR", "Nama Database tidak boleh kosong!")
                return
            ...
            if not self.user_cursor.fetchone():
                messagebox.showerror("GAGAL", "Verifikasi Admin Gagal!")
                return

            # Buat database baru
            if new_db_name in DB_MAPPING:
                messagebox.showerror("GAGAL", "Nama Database sudah ada!")
                return

            file_name = new_db_name.lower().replace(" ", "_") + ".db"
            DB_MAPPING[new_db_name] = file_name
            save_db_mapping(DB_MAPPING)

            messagebox.showinfo("SUKSES", f"Database '{new_db_name}' berhasil ditambahkan!")
            
            # Refresh dropdown di UI Database Selector
            self.db_options = list(DB_MAPPING.keys())
            self.dropdown.configure(values=self.db_options)
            self.db_choice.set(new_db_name)
            win.destroy()

        ctk.CTkButton(win, text="TAMBAHKAN", fg_color="#388E3C", hover_color="#2E7D32", command=verify_and_add).pack(pady=20)
    ...
    def main_ui(self):
        ...
        if self.role == "admin":
            ...
            dev_frame = ctk.CTkFrame(self.root, fg_color="#E3F2FD", corner_radius=8)
            dev_frame.pack(fill=X, padx=10, pady=5)
            ctk.CTkLabel(dev_frame, text="🔧 DEV TOOLS:", font=("Segoe UI", 11, "bold"), text_color="#1565C0").pack(side=LEFT, padx=10)
            ctk.CTkButton(dev_frame, text="CEK UPDATE (GITHUB)", fg_color="#1565C0", command=self.check_update, **btn_kwargs).pack(side=LEFT, padx=5)
            ctk.CTkButton(dev_frame, text="EDIT SOURCE CODE", fg_color="#4527A0", command=self.edit_source_code, **btn_kwargs).pack(side=LEFT, padx=5)
    ...
    # --- FITUR DEV / MAINTENANCE ---
    def check_update(self):
        # Karena kita tidak tahu repository aslinya, arahkan ke halaman utama/instruksi.
        url = "https://github.com/username-anda/nama-repo-anda/releases"
        confirm = messagebox.askyesno("CEK UPDATE", "Fitur ini akan membuka halaman Repository GitHub untuk mengunduh versi terbaru.\n\nLanjutkan ke Browser?")
        if confirm:
            webbrowser.open(url)

    def edit_source_code(self):
        msg = ("PERHATIAN: Fitur ini hanya akan berfungsi jika aplikasi ini dijalankan "
               "sebagai script Python murni (.py).\n\n"
               "Jika Anda sudah menjadikan aplikasi ini sebagai (.exe), Anda tidak dapat "
               "mengedit kodenya secara langsung. Anda harus mengedit file asal (.py) dan mem-build ulang.\n\n"
               "Apakah Anda ingin mencoba membuka source code?")
        if messagebox.askyesno("MAINTENANCE MODE", msg):
            try:
                # Membuka file saat ini dengan editor teks default OS
                os.startfile(os.path.abspath(__file__))
            except Exception as e:
                messagebox.showerror("ERROR", f"Gagal membuka kode sumber.\nAlasan: {str(e)}")
    ...
    def open_calendar_filter(self):
        ...
        cal = Calendar(cal_win, selectmode='day', date_pattern='y-mm-dd', showweeknumbers=False)
    ...
    def keep_focus(self):
        try:
            if (self.is_running and not self.popup_open and hasattr(self, "scan_entry") 
                and self.scan_entry.winfo_exists() and self.root.focus_get() != self.scan_entry):
                self.scan_entry.focus_force()
        except Exception:
            pass
        if self.is_running and hasattr(self, "scan_entry"):
    ...
    def search_data(self):
        ...
        def do_search():
            try:
                ...
                if self.current_date_filter:
                    self.cursor.execute("SELECT id, unique_id, barcode, status, username, waktu FROM scans WHERE barcode LIKE ? AND waktu LIKE ?", (f"%{key}%", f"{self.current_date_filter}%"))
                else:
                    self.cursor.execute("SELECT id, unique_id, barcode, status, username, waktu FROM scans WHERE barcode LIKE ?", (f"%{key}%",))
                ...
                for idx, row in enumerate(self.cursor.fetchall(), 1):
                    ...
                    self.cursor.execute("SELECT id, unique_id, username, waktu FROM scans WHERE barcode=? AND waktu LIKE ? ORDER BY id ASC LIMIT 1", (barcode, f"{date_str}%"))
                    first_dt = self.cursor.fetchone()
                    
                    ket, tags = ("", ())
                    if first_dt and first_dt[0] != row_id:
                        ref_unique = first_dt[1] if first_dt[1] else f"LEGACY-{first_dt[0]}"
                        ket = f"⚠ DOUBLE DENGAN ID {ref_unique} | {first_dt[2]} | {first_dt[3]}"
                        status = "DOUBLE"
                        tags = ("double",)
                    else:
                        status = "NORMAL"
    ...
    def show_about_popup(self):
        ...
        title_lbl = ctk.CTkLabel(main_frame, text="Resi Scanner V1", font=("Segoe UI", 22, "bold"), text_color="#1A237E")
        ...
        create_section("versi", "2.0.0")
        create_section("Jenis Engine yang di gunakan", "Sqllite, customtkinter, tkcalendar, pandas, openpyxl, reportlab")
        create_section("Troubleshooting", "Jika terjadi error, silahkan cek file error.log di folder yang sama dengan aplikasi ini. Jika tidak ada, silahkan hubungi developer.")
        create_section("Thanks to", "Allah SWT., GitHub, W3schools, @ar1nben1, fromwire, @WeebDeveloperBDG, & Papaya Fresh Gallery Bandung")
        
        footer_lbl = ctk.CTkLabel(main_frame, text="@2026\nseftian_permana. (tidak berlisensi alias silahkan di gunakan secara bebas)", font=("Segoe UI", 11, "bold"), text_color="#37474F", justify="center")
        footer_lbl.pack(pady=(15, 10))
        
        close_btn = ctk.CTkButton(main_frame, text="TUTUP", width=100, height=30, fg_color="#5C6BC0", hover_color="#3949AB", font=("Segoe UI", 11, "bold"), command=about_win.destroy)
